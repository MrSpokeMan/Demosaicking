...
img = cv2.imread("./img/4demosaicking.bmp")
img_x = cv2.imread("./img/4demosaicking.bmp")
...
Rx, Gx, Bx = [cv2.filter2D(img_x[..., n], -1, deTransFilter[n]) for n in range(3)]
...
for i in range(height):
    for j in range(width):
        img[i][j] = [(img[i][j][0] - rgb[i][j][0]), (img[i][j][1] - rgb[i][j][1]), (img[i][j][2] - rgb[i][j][2])]
        
cv2.imwrite("./img/diff_bayer.bmp", img)

for i in range(height):
    for j in range(width):
        img_x[i][j] = [(img_x[i][j][0] - rgb_x[i][j][0]), (img_x[i][j][1] - rgb_x[i][j][1]), (img_x[i][j][2] - rgb_x[i][j][2])]
